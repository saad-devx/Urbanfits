...
const getCoupon = async (req, res) => {
    try {
        ...
        if (!coupon_code || coupon_code.length < 8) res.status(500).json({ success: false, error, msg: "Invalid coupon code." })
        ...
        if (req.method === 'GET') {
            ...
            if (!coupon) return res.status(500).json({ success: false, error, msg: "Invalid coupon code." })
            delete coupon._id

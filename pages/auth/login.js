        ...
        <AuthPage loading={userLoading} mblNav="/auth/signup" mblNavName={langObj.signup} langObj={langObj}>
            <form className="w-full h-full lg:h-auto bg-white p-2 lg:p-0 font_urbanist text-base flex flex-col justify-between md:justify-around lg:block" onReset={handleReset} onSubmit={handleSubmit} >
                <section className="w-full mb-6 md:mb-0">
                    <h1 className="lg:hidden text-[22px] mb-5 text-left font_urbanist">{langObj.login}</h1>
                    <div className={`relative data_field lex items-center border-b ${touched.new_email && errors.new_email ? "border-red-500" : "focus:border-pink-300"} transition py-2 mb-4`}>
                        ...
                        <input className="w-full outline-none border-none" name="email" id="email" value={values.email} onBlur={handleBlur} onChange={handleChange} placeholder={langObj.usernameOrEmail} />
                    </div>
                    <div className={`relative data_field flex items-center border-b ${touched.new_email && errors.new_email ? "border-red-500" : "focus:border-pink-300"} transition py-2 mb-4`}>
                        ...
                        <input ref={passRef} className={`w-full outline-none border-none ${values.password ? "tracking-2" : null}`} type={showPass ? "text" : "password"} id="password" value={values.password} onBlur={handleBlur} onChange={handleChange} placeholder={langObj.password} />
                        <i onClick={() => {
                    ...
                    <div className="relative w-full mb-2 flex items-center">
                        ...
                        <div className="w-full flex justify-between items-center text-gray-400 text-sm">
                            <div className='flex items-center gap-x-2'>
                                ...
                                <label htmlFor='remember_me' className="w-full -translate-y-0.5 cursor-pointer text-gray-400">{langObj.rememberMe}</label>
                            </div>
                            <Link href="/auth/resetpassword">{langObj.forgotPass}</Link>
                        </div>
                    </div>
                    <Button loading={userLoading} my="my-4" classes='w-full' type="submit">{langObj.login}</Button><div className="lg:hidden w-full flex justify-between items-center font_urbanist text-sm">
                        <span className="w-2/5 h-px bg-gray-200"></span>
                        login via
                        <span className="w-2/5 h-px bg-gray-200"></span>
                    </div>
                    <Link href='/auth/signup' className='hidden lg:block underline text-xs md:text-sm'><h1 className='w-full text-center'>{langObj.createAccount}</h1></Link>

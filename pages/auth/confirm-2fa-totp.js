        ...
        <AuthPage loading={userLoading} mblNav="/auth/signup" mblNavName={langObj.signup} langObj={langObj}>
            <main className="w-full h-full lg:h-auto bg-white p-2 lg:p-0 font_urbanist text-base flex flex-col justify-between md:justify-around lg:block" >
                <section className="w-full mb-6 md:mb-0">
                    <h1 className="lg:hidden text-[22px] mb-5 text-left font_urbanist">{langObj.comfirmTotp}</h1>
                    <div className="relative data_field lex items-center border-b focus:border-pink-300 transition py-2 mb-4">
                        <input className="w-full outline-none border-none" type='number' name="totp" id="totp" value={totp} onChange={(e) => setTotp(e.target.value)} placeholder={langObj.enterTotp} />
                    </div>
                    <div className="my-3 text-gray-400 text-xs md:text-sm text-left">
                        {langObj.totpInstruction}
                    </div>
                </section>
                <Button disabled={!totp || totp.length < 5 || totp.length > 10} onClick={() => onVerifyClick(router.query.user_id)} loading={userLoading} my="my-4" classes='w-full'>{langObj.verify}</Button>
            </main>
        </AuthPage>
    </>
}
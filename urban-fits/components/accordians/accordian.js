...
export default function Accordian (props) {
    ...
    return (
        <div className="p-6 outline-none accordion-section rounded-xl bg-gray-50 mb-6" tabIndex={1}>
            ...
            <div className={` ${open ? 'max-h-screen mt-5' : 'max-h-0'} text-sm leading-5 rounded overflow-y-hidden scrollbar_x ease duration-700`}>
                <div className="flex items-center mb-3 py-2">
                    <div className='font_gotham_light text-sm text-black'>{props.children}</div>
                </div>
            </div>
        </div>
    )

...
export default function Accordian(props) {
    ...
    return <div className="p-4 lg:p-6 outline-none accordion-section rounded-xl bg-gray-50 mb-6" tabIndex={1}>
        <div onClick={() => setOpen(!open)} className={`${open ? 'border-b' : null} flex justify-between h-8 items-center transition ease duration-700 cursor-pointer relative`}>
        ...
        <div className={` ${open ? 'max-h-screen mt-5' : 'max-h-0'} relative max-w-full text-sm leading-5 rounded overflow-y-hidden scrollbar_x ease duration-700`}>
            <div id='Accordian_Container' className="flex items-center mb-3 py-2">
                <div className='xl:w-full font_gotham_light text-sm text-black'>{props.children}</div>
            </div>
        </div>
    </div>
}
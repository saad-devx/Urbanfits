...
export default function SizeTableAccordian(props) {
    ...
    const TableRow = (array, indexColWidth, restColsWidth, dataIndex, dataLength) => {
        ...
        if (typeof (array[1]) === 'object') return array.map((size, index) => {
            return <span key={index} className={`${index == 0 ? `${indexColWidth} sticky left-0 bg-gray-100 lg:bg-gray-50 pl-2 font_gotham_medium text-left text-[10px] md:text-sm` : `${restColsWidth} ${props.bigFontOf_nthRow && props.bigFontOf_nthRow.includes(dataIndex) ? 'text-xs' : 'text-[10px]'} font_urbanist`} ${index == 0 && dataIndex === dataLength - 1 ? 'rounded-bl-md' : null} ${dataIndex === dataLength - 1 ? null : 'border-b'} py-3 text-black`}>{index === 0 ? size : sizeVal(size)}</span>
        })
        else return array.map((size, index) => {
            return <span key={index} className={`${index == 0 ? `${indexColWidth} sticky left-0 bg-gray-100 lg:bg-gray-50 pl-2 font_gotham_medium text-left text-[10px] md:text-sm` : restColsWidth} ${index == 0 && dataIndex === dataLength - 1 ? 'rounded-bl-md' : null} ${dataIndex === dataLength - 1 ? null : 'border-b'} py-3 text-black`}>{size}</span>
        })
    }
    return (
                    ...
                    <div className={`${props.containerWidth ? props.containerWidth : 'sm:w-full'} font_gotham_light text-sm text-black`}>
                        <section className={`max-w-[2000px] rounded-md ${props.containerWidth ? props.containerWidth : 'sm:w-full'} flex flex-col justify-center bg-gray-50`}>
                            <div className="flex text-center font_gotham_bold">
                                {props.tableHeading ? <span className={`${props.indexColWidth} sticky left-0 bg-gray-100 lg:bg-gray-50 xl:bg-inherit pl-2 pt-4 lg:pt-3 rounded-tl-md border-b pr-4 py-3 text-[10px] md:text-sm text-left text-black ${props.headingTracking_null ? '' : 'tracking-expand'}`}>{props.tableHeading}</span> : null}

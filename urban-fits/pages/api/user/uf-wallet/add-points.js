...
const AddUFpoints = async (req, res) => {
    try {
        ...
        if (req.method === 'PUT') {
            ...
            const { card_number, user_id, points, source } = decodedToken
            if (!card_number || !user_id || !points || !source) return res.status(403).json({ success: false, msg: "Invalid token arguments." })
            await ConnectDB()

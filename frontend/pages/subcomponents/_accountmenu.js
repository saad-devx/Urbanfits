...
const Option = (props) => {
    ...
    return (
        <Link className={` group w-full h-[11%] flex justify-between items-center mb-[2px] pr-3 text-sm rounded-sm bg-white hover:translate-x-1 transition-all `} href={props.href}><span className={`bg-gold w-2 h-full opacity-0 group-hover:opacity-100 ${route === props.href ? 'opacity-100' : ''} transition`}></span>{props.value}<i className=" arrow material-symbols-outlined text-lg text-gray-600 transition-all">chevron_right</i></Link>
    )
}

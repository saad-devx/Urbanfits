    ...
    return (
        <SplideSlide className={`${props.size} relative  p-10`}>
            <div className="absolute w-3/12 bottom-[7%] left-[2%] flex flex-col items-start text-white text-5xl">
                ...
                <LinkBtn my="my-0" value="Shope Now" bg="bg-white" text="black" classes="w-full text-xs md:text-base" />
            </div>
        ...
        <div className={` ${props.classes} font_futuraLT transition-all duration-1000 overflow-hidden`}>
            <Splide className={`w-full h-full relative transition-all duration-1000`} onPaginationMounted={(data) => { data.list.classList.add('hidden') }} hasTrack={false} aria-label="My Favorite Images"
                ...
                <div className="splide__arrows absolute flex space-x-5 bottom-12 right-10">
                    ...
                    <button onClick={togglePlay} className="splide__toggle flex justify-center items-center w-8 h-8 rounded-full bg-white" >
                        {play === false ? <i className="material-symbols-rounded fill">play_arrow</i>
                            : <svg width="8" height="12" viewBox="0 0 8 12" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <line x1="1.25" y1="12" x2="1.25" stroke="black" stroke-width="1.5" />
                                <line x1="7.25" y1="12" x2="7.25" stroke="black" stroke-width="1.5" />
                            </svg>}
                    </button>

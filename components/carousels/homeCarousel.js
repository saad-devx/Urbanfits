...
import placeholederImg from "../../public/carousel-placeholder.webp"
...
export default function HomeCarousel({ homeSlides, getHomeSlides }) {
    ...
    return (
        <div className="w-full h-full relative">
            {(!homeSlides?.length || !isLoaded) && (
                <Image
                    src={placeholederImg}
                    alt="Loading..."
                    fill
                    priority
                    className="object-cover z-50 absolute inset-0"
                    placeholder="blur"
                />
            )}

            {homeSlides?.length > 0 && (
                <Splide fixed className="w-full h-full relative font_urbanist transition-all duration-1000" hasTrack={false}
                    options={{
                        type: 'loop',
                        speed: 900,
                        gap: '0.5rem',
                        ...
                        {homeSlides.map((slide, index) => <SplideSlide key={index} className="w-full h-full">
                            <Link className="w-full h-full relative" href={slide.href || "#"}>
                                ...
                                <Image
                                    ...
                                    width={1500}
                                    height={650}
                                    src={slide.local ? slide.image : process.env.NEXT_PUBLIC_BASE_IMG_URL + slide.image}
                                    priority={index === 0}
                                    alt={slide.title}
                                    onLoad={() => { if (index === 0) setIsLoaded(true) }}
                                />
                            </Link>
                        </SplideSlide>)}
                    </SplideTrack>
                    ...
                    <div className="splide__arrows absolute flex items-center gap-x-5 right-[3%] bottom-[12%] lg:bottom-[10%]">
                        <button className="splide__arrow--prev hover:bg-pink-200 flex justify-center items-center w-8 h-8 rounded-full bg-white rotate-180 transition-all duration-500" >
                            <span className="w-3 md:w-5 aspect-square">
                                <svg className='w-full' width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path fillRule="evenodd" clipRule="evenodd" d="M1.75 12.0001C1.75 12.4143 2.08579 12.7501 2.5 12.7501L21.5 12.7501C21.9142 12.7501 22.25 12.4143 22.25 12.0001C22.25 11.5858 21.9142 11.2501 21.5 11.2501L2.5 11.2501C2.08579 11.2501 1.75 11.5858 1.75 12.0001Z" fill="#BFBFBF" />
                                    <path fillRule="evenodd" clipRule="evenodd" d="M1.75 12.0001C1.75 12.199 1.82902 12.3897 1.96967 12.5304L8.96967 19.5304C9.26256 19.8233 9.73744 19.8233 10.0303 19.5304C10.3232 19.2375 10.3232 18.7626 10.0303 18.4697L3.56066 12.0001L10.0303 5.53039C10.3232 5.2375 10.3232 4.76262 10.0303 4.46973C9.73744 4.17684 9.26256 4.17684 8.96967 4.46973L1.96967 11.4697C1.82902 11.6104 1.75 11.8011 1.75 12.0001Z" fill="black" />
                                </svg>
                            </span>
                        </button>
                        ...
                        <button className="splide__arrow--next hover:bg-pink-200 flex justify-center items-center w-8 h-8 rounded-full bg-white transition-all duration-500" >
                            <span className="w-3 md:w-5 aspect-square">
                                <svg className='w-full' width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path fillRule="evenodd" clipRule="evenodd" d="M22.25 12.0001C22.25 12.4143 21.9142 12.7501 21.5 12.7501L2.5 12.7501C2.08579 12.7501 1.75 12.4143 1.75 12.0001C1.75 11.5859 2.08579 11.2501 2.5 11.2501L21.5 11.2501C21.9142 11.2501 22.25 11.5858 22.25 12.0001Z" fill="#BFBFBF" />
                                    <path fillRule="evenodd" clipRule="evenodd" d="M22.25 12.0001C22.25 12.199 22.171 12.3897 22.0303 12.5304L15.0303 19.5304C14.7374 19.8233 14.2626 19.8233 13.9697 19.5304C13.6768 19.2375 13.6768 18.7626 13.9697 18.4697L20.4393 12.0001L13.9697 5.53039C13.6768 5.2375 13.6768 4.76263 13.9697 4.46973C14.2626 4.17684 14.7374 4.17684 15.0303 4.46973L22.0303 11.4697C22.171 11.6104 22.25 11.8011 22.25 12.0001Z" fill="black" />
                                </svg>
                            </span>
                        </button>
                    </div>

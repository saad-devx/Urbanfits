...
export default function Search({ locale, classes, noResultsMsg, placeholder }) {
    ...
    return <div className={`${classes} relative z-50 w-full md:w-1/3 xl:w-2/5 h-9 lg:h-10 px-6 justify-between items-center font_urbanist bg-gray-100 text-xs md:text-sm rounded-full`}>
        <input type="text" name='search_box' role='search' value={query} onChange={onSearch} className='w-full bg-transparent border-none outline-none' placeholder={placeholder} />
        ...
        {query !== '' ? <div className="absolute top-full right-0.5 translate-y-1 w-full min-h-[5rem] p-2 bg-white border rounded-md shadow-md">
            {loader}
            {(results.length === 0 || !results) && !loader ? <span className="w-full h-[4rem] flex justify-center items-center font_urbanist_medium text-xs">{noResultsMsg}</span> : null}
            {results.map((result, index) => {
                ...
                else return <Link key={index} onClick={() => setQuery('')} className={`${results.length === index + 1 ? null : "border-b"} w-full h-10 flex items-center font-urbanist text-xs capitalize`} href={`/products/category/${result._id}?name=${result.name[locale].toLowerCase()}`}>

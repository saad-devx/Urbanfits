...
import { CrossIcon } from '@/public/accountIcons';
    ...
    if (props.subLinks) return <div {...props} className="border-b border-gray-50">
        ...
        <div className={`${open ? 'max-h-[50vh]' : 'max-h-0'} w-full text-sm overflow-y-hidden transition-all duration-500`}>
            {props.subLinks.map((link, i) => {
                if (link.comingSoon) return <div key={i} onClick={() => props.toggleMenu()} className='w-full pb-4 pl-8 font_urbanist text-black flex justify-between items-center'>
...
function MobileNavbar({ isLoggedIn, cart, toggleCart, logout, setLogout, totalUniqueItems }) {
    ...
    if (!Exception) return <>
        ...
        <section className="fixed z-[60] bottom-4 left-1/2 -translate-x-1/2 bg-gray-50 w-[90%] md:w-3/5 h-14 rounded-full border flex justify-around items-center">
            ...
            <button onClick={toggleCart} className="relative">
                ...
                {cart ? <CrossIcon /> : <svg width="20" height="21" viewBox="0 0 20 21" fill="none" xmlns="http://www.w3.org/2000/svg">

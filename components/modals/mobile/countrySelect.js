    ...
    return <section style={{ transform: show ? "translateX(-100%)" : "translateX(0)" }} className='fixed z-50 top-0 -right-full w-full h-screen overflow-y-scroll bg-white flex flex-col transition-all duration-500'>
        ...
        <div className="w-full flex flex-col">
            {filteredCountries.map((c, i) => {
                return <button onClick={() => { setCountry(c); setCoutnryModal(false); useUser.setState({ geo_selected_by_user: true }) }} key={i} className={`w-full ${filteredCountries.length === i + 1 && "mb-20"} p-4 flex justify-between items-center ${country.country === c.country ? "bg-gray-50 border border-gray-400 rounded-lg" : "border-b border-gray-50"} font_urbanist text-base capitalize`}>
